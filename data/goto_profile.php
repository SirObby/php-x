...
<body>
    ...
    <nav>
        ...
        if(!session('access_token')) {
            ...
            if($did_work) {
                echo '<a href="/profile.php">@' . pg_fetch_result($results,0,"handle") . "</a>";
            }

        }

        $query00 = "SELECT * FROM users WHERE handle='" . $_POST["id"] . "';";
            $res = pg_query($dbconn, $query00) or $did_work = false;
        ...
        <tr>
            ...
            $did_work;
            $query = "SELECT * FROM tweets WHERE user_id=" . pg_fetch_result($res,0,"id") . ";";
            $results = pg_query($dbconn, $query) or $did_work = false;
